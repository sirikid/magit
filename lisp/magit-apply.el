...
(defun magit-apply-patch (section:s args patch)
  (let* ((files (if (atom section:s)
                  ...
                  (seq-map (lambda (it) (oref it value)) section:s)))
...
(defun magit-discard-files--discard (sections new-files)
  (let ((files (seq-map (lambda (it) (oref it value)) sections)))
      ...
      (let ((binaries (magit-binary-files "--cached")))
        ...
        (when binaries
          (let* ((modified (magit-unstaged-files t))
                 (groups (seq-group-by (lambda (it)
                                         (member it modified))
                                       binaries)))
            (when (assq nil groups)
...
(defun magit-reverse-files (sections args)
  (let* ((bs (magit-binary-files
         ...
         (groups (seq-group-by (lambda (it)
                               ...
                               sections))
         (binaries (assq t groups))
         (sections (assq nil groups)))
    (magit-confirm-files 'reverse (seq-map (lambda (it)
                                             (oref it value))
                                           sections))
    (cond ((= (length sections) 1)
