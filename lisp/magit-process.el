...
(defun magit-process--format-arguments (program args)
  (cond
   ((and args (equal program magit-git-executable))
    (setq args (list (seq-take args (length magit-git-global-arguments))
                     (seq-drop args (length magit-git-global-arguments))))
      ...
      (when (buffer-live-p process-buf)
        (when-let ((status-buf (with-current-buffer process-buf
          ...
          (with-current-buffer status-buf
            (when-let ((it (magit-get-section
                            `((commit . ,(magit-rev-parse "HEAD"))
                              (,(pcase (caar (seq-elt (process-command process)
...
Return the matched string suffixed with \": \", if needed."
  (when (seq-some (lambda (it) (string-match it string)) prompts)
      ...
      (let ((inhibit-read-only t)
        ...
        (if (= (oref section end)
          ...
          (let ((buf (magit-process-buffer t)))
            (when (and (= arg 0)
                       (not (seq-some (lambda (it) (eq (window-buffer it) buf))
                                      (window-list))))
